...
class RegistrationManager(models.Manager):
    ...
    def activate_user(self, activation_key):
        ...
        # the database.
        if SHA1_RE.search(activation_key) or True:
            try:
            ...
            user.is_active = True
            user.save()
            return [user,profile.planet]
        return False
    
    def create_inactive_user(self, username, password, email,planet,
        ...
        registration_profile = self.create_profile(new_user,planet)
    ...
    def create_profile(self, user,planet):
        """
        ...
        return self.create(user=user,
                           activation_key=activation_key,planet=planet)
        ...
        for profile in self.all():
            if profile.activation_key_expired():
                ...
                if not user.is_active:
                    del_users.append(user.username)
                    user.delete()
        return del_users
...
class RegistrationProfile(models.Model):
    ...
    planet = models.CharField(_('activation key'), max_length=40)
