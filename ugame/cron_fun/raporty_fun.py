...
def rap_spy(dane_raport, flota):
    ...
    tekst_agresor = render_to_string("ugame/reports_strings/spy_agresor.html", dane_raport)
    tekst_obronca = render_to_string("ugame/reports_strings/spy_obronca.html", dane_raport)
...
def rap_zlom(flota, GraObj):
    ...
    tekst = render_to_string("ugame/reports_strings/zlom.html", dane_raport)
...
def rap_przeslij(flota, statki_powrot, GraObj):
    ...
    koordy_do = '%s:%s:%s' % (flota.galaxy_end.galaxy, flota.galaxy_end.system, flota.galaxy_end.field)
    data = datetime.datetime.fromtimestamp(flota.time)
    tekst = render_to_string("ugame/reports_strings/przeslij.html", dane_raport)

    rap = Raporty.objects.create(typ='P', user=GraObj.user, tytul="Flota z misja prześlij dla planety " + koordy_do,
                                 tekst=tekst,
                                 koordy_z=koordy_z, koordy_do=koordy_do, data=data)
...
def rap_atak(dane_raport, flota, wygrana):
    ...
    tekst_obronca = render_to_string("ugame/reports_strings/atak.html", dane_raport)
...
def kolonizajca_fail_planety(flota, GraObj):
    ...
    tekst = render_to_string("ugame/reports_strings/kolonizacja_fail.html", dane_raport)
    koordy_z = '%s:%s:%s' % (flota.galaxy_start.galaxy, flota.galaxy_start.system, flota.galaxy_start.field)
    koordy_do = '%s:%s:%s' % (flota.galaxy_end.galaxy, flota.galaxy_end.system, flota.galaxy_end.field)
    data = datetime.datetime.fromtimestamp(flota.time)
...
def kolonizacja_fail_ktoinny(flota, GraObj):
    GraObj.userprofile.nowe_raporty += 1
    dane_raport = {"galaxy": flota.galaxy_end, "flota": flota}
    tekst = render_to_string("ugame/reports_strings/kolonizacja_fail_ktoinny.html", dane_raport)
    koordy_z = '%s:%s:%s' % (flota.galaxy_start.galaxy, flota.galaxy_start.system, flota.galaxy_start.field)
    koordy_do = '%s:%s:%s' % (flota.galaxy_end.galaxy, flota.galaxy_end.system, flota.galaxy_end.field)
    data = datetime.datetime.fromtimestamp(flota.time)
...
def kolonizacja_ok(flota, GraObj):
    GraObj.userprofile.nowe_raporty += 1
    dane_raport = {"galaxy": flota.galaxy_end, "flota": flota}
    tekst = render_to_string("ugame/reports_strings/kolonizacja_ok.html", dane_raport)
    koordy_z = '%s:%s:%s' % (flota.galaxy_start.galaxy, flota.galaxy_start.system, flota.galaxy_start.field)
    koordy_do = '%s:%s:%s' % (flota.galaxy_end.galaxy, flota.galaxy_end.system, flota.galaxy_end.field)
    data = datetime.datetime.fromtimestamp(flota.time)
