...
def atak(request):
    ...
    return jrender_response(request,'game/fs_atak.html',{"dane":dane,'topnav':topnav,"dostepne_statki":dostepne_statki})
...
def atak_1(request):
    ...
    return jrender_response(request,'game/fs_atak_podsumowanie.html',{"planeta_dane":planeta_dane,"dane_floty":dane_floty,'topnav':topnav,"statki_podsumowanie":statki_podsumowanie})
...
def atak_2(request):
    ...
    flota_row = Fleets.objects.create(fleet_owner=GraObject.user,fleet_mission=4,fleet_amount=fleet_amount,fleet_array=";".join(fleet_array),time_start=time_start,
