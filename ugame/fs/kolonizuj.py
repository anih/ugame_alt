...
def kolonizuj(request):
    ...
    if not fun.check_free_planet(GraObject.user, dane.planeta_id, dane.system_id, dane.galaktyka_id):
        return HttpResponseRedirect("/game/galaxy/")
    ...
    return jrender_response(request,'game/fs_kolonizuj.html',{"dane":dane,'topnav':topnav,"dostepne_statki":dostepne_statki})


@check_active
def kolonizuj_1(request):
    GraObject = request.GameObj
    current_planet = GraObject.get_current_planet()
    ...
    if not fun.check_free_planet(GraObject.user, planeta_dane.planeta_id, planeta_dane.system_id, planeta_dane.galaktyka_id):
        
        return HttpResponseRedirect("/game/galaxy/")
    if not fun.check_mamy_kolonizator(GraObject.user,current_planet):
    ...
    if not id_statkow or not ship_request:
        tmp = fun.get_ship_request(GraObject,current_planet, request, GraObject.user,planeta_dane,True)
    else:
        tmp = fun.get_ship_request(GraObject,current_planet, request, GraObject.user,planeta_dane,True,False,False)
    
    if not tmp:
        if not id_statkow or not ship_request:
            
            return HttpResponseRedirect("/game/fs/kolonizuj/")
        else:
    ...
    return jrender_response(request,'game/fs_kolonizuj_podsumowanie.html',{"planeta_dane":planeta_dane,"dane_floty":dane_floty,'topnav':topnav,"statki_podsumowanie":statki_podsumowanie})


@check_active
def kolonizuj_2(request):
    GraObject = request.GameObj
    ...
    tmp = fun.check_ship_request(GraObject,request,current_planet,planeta_dane,id_statkow, ship_request, GraObject.user,True)
    if not tmp:
    ...
    tmp = fun.check_surowce(request,current_planet,dane_floty)
       
    if not tmp:
        
        return HttpResponseRedirect("/game/fs/kolonizuj/1/")
    
    zab_met,zab_cry,zab_deu=tmp

    current_planet.deuter -= zab_deu+dane_floty.zuzycie_deuter
    current_planet.crystal -= zab_cry
    current_planet.metal -= zab_met
    ...
    flota_row = Fleets.objects.create(fleet_owner=GraObject.user,fleet_mission=1,fleet_amount=fleet_amount,fleet_array=";".join(fleet_array),time_start=time_start,
                       galaxy_start=current_planet.galaxy,
                       time=time_start+dane_floty.czas_lotu,
                       galaxy_end=galaxy_end,
                       fleet_resource_metal=zab_met,fleet_resource_crystal=zab_cry,fleet_resource_deuterium=zab_deu,bak=dane_floty.zuzycie_deuter)
