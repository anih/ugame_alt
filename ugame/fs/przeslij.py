...
def przeslij(request):
    ...
    if not fun.is_planet_owner(GraObject.user,dane):
    ...
    return jrender_response(request,'game/fs_przeslij.html',{"dane":dane,'topnav':topnav,"dostepne_statki":dostepne_statki})


@check_active
def przeslij_1(request):
    GraObject = request.GameObj
    current_planet = GraObject.get_current_planet()
    ...
    if not fun.check_is_planet(GraObject.user, planeta_dane.planeta_id, planeta_dane.system_id, planeta_dane.galaktyka_id):
        return HttpResponseRedirect("/game/galaxy/")
    if not fun.is_planet_owner(GraObject.user,planeta_dane):
        return HttpResponseRedirect("/game/overview/")
    ...
    topnav = topnav_site(GraObject)
    return jrender_response(request,'game/fs_przeslij_podsumowanie.html',{"planeta_dane":planeta_dane,"dane_floty":dane_floty,'topnav':topnav,"statki_podsumowanie":statki_podsumowanie})


@check_active
def przeslij_2(request):
    GraObject = request.GameObj
    current_planet = GraObject.get_current_planet()
    ...
    if not tmp:
        return HttpResponseRedirect("/game/fs/przeslij/1/")
    ...
    flota_row = Fleets.objects.create(fleet_owner=GraObject.user,fleet_mission=3,fleet_amount=fleet_amount,fleet_array=";".join(fleet_array),time_start=time_start,
