...
def surowce(request):
    ...
    return jrender_response(request,'game/fs_surowce.html',{"dane":dane,'topnav':topnav,"dostepne_statki":dostepne_statki})


@check_active
def surowce_1(request):
    GraObject = request.GameObj
    current_planet = GraObject.get_current_planet()
    ...
    try:
        ...
        return HttpResponseRedirect("/game/fs/surowce/")
    ...
    topnav = topnav_site(GraObject)
    return jrender_response(request,'game/fs_surowce_podsumowanie.html',{"planeta_dane":planeta_dane,"dane_floty":dane_floty,'topnav':topnav,"statki_podsumowanie":statki_podsumowanie})


@check_active
def surowce_2(request):
    GraObject = request.GameObj
    current_planet = GraObject.get_current_planet()
    ...
    if not tmp:
        return HttpResponseRedirect("/game/fs/surowce/1/")
    ...
    flota_row = Fleets.objects.create(fleet_owner=GraObject.user,fleet_mission=2,fleet_amount=fleet_amount,fleet_array=";".join(fleet_array),time_start=time_start,
                       galaxy_start=current_planet.galaxy,
                       time=time_start+dane_floty.czas_lotu,
                       galaxy_end=galaxy_end,
                       fleet_resource_metal=zab_met,fleet_resource_crystal=zab_cry,fleet_resource_deuterium=zab_deu,
