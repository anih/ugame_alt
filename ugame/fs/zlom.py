...
def zlom(request):
    ...
    dostepne_statki_tmp = Flota_p.objects.values_list('budynek_id',flat=True).filter(planeta=current_planet,budynek__lata__gt=0,ilosc__gt=0,budynek__recycler__isnull=False).order_by("budynek")
    dostepne_statki = []
    ...
    return jrender_response(request,'game/fs_zlom.html',{"dane":dane,'topnav':topnav,"dostepne_statki":dostepne_statki})
...
def zlom_1(request):
    ...
    if not fun.check_mamy_recycler(GraObject.user,current_planet):
        return HttpResponseRedirect("/game/overview/")
    ...
    if not id_statkow or not ship_request:
        tmp = fun.get_ship_request(GraObject,current_planet, request, GraObject.user,planeta_dane,False,True)
    else:
        tmp = fun.get_ship_request(GraObject,current_planet, request, GraObject.user,planeta_dane,False,True,False)
    
    if not tmp:
        if not id_statkow or not ship_request:
            return HttpResponseRedirect("/game/fs/zlom/")
        else:
    ...
    topnav = topnav_site(GraObject)
    return jrender_response(request,'game/fs_zlom_podsumowanie.html',{"planeta_dane":planeta_dane,"dane_floty":dane_floty,'topnav':topnav,"statki_podsumowanie":statki_podsumowanie})
...
def zlom_2(request):
    ...
    tmp = fun.check_ship_request(GraObject,request,current_planet,planeta_dane,id_statkow, ship_request, GraObject.user,False,True)
    if not tmp:
        return HttpResponseRedirect("/game/overview/")
    dane_floty=tmp['dane_floty']
    id_statkow=tmp['id_statkow']
    ...
    flota_row = Fleets.objects.create(fleet_owner=GraObject.user,fleet_mission=5,fleet_amount=fleet_amount,fleet_array=";".join(fleet_array),time_start=time_start,
