...
def getCzasLotu(game_speed, speed, distance, fleet_speed):
    ...
    print "test:", game_speed, speed, distance, fleet_speed
...
def is_planet_owner(user, planeta_dane, odwrotne=False):
    galaxy = Galaxy.objects.get(galaxy=planeta_dane.galaktyka_id, system=planeta_dane.system_id, field=planeta_dane.planeta_id)
...
def get_zuzycie_deuteru(dane, planeta_dane):
    ...
    zuzycie_deuter = int((dane.basic_consumption / 8) * (dane.odleglosc / 35000.0) * ((spd / 10.0) + 1) * ((spd / 10.0) + 1.0))
    return zuzycie_deuter * (planeta_dane.speed_procent / 10) + 2

def get_ship_request(GraObject, planeta, request, user, planeta_dane, check_kolonizacja=None, check_recycler=None, wiadomosci=True):
    kolonizacyjny = False
    recyclerski = False
    bledne_ilosci = False
    ...
    statki_podsumowanie = []
    ship_request = {}
    wszystkie_statki = Flota_p.objects.values_list("budynek_id", flat=True).select_for_update().filter(planeta=planeta, budynek__lata__gt=0, ilosc__gt=0).order_by("budynek")
    for tmp in wszystkie_statki:
        i = GraObject.cache_obj.get_flota_p(planeta.pk, tmp)
        if request.REQUEST.has_key('ship_' + str(i.pk)):
            try:ilosc = int(request.REQUEST['ship_' + str(i.pk)])
            except:ilosc = 0
...
def check_ship_request(GraObject, request, planeta, planeta_dane, id_statkow, ship_request, user, check_kolonizacja=None, check_recycler=None):
    kolonizacyjny = False
    recyclerski = False
    bledne_ilosci = False
    jakikolwiek_statek = False
    ...
    dane_floty.basic_consumption = 0
    statki_podsumowanie = []
    statki_do_wyslania = Flota_p.objects.values_list("budynek_id", flat=True).filter(planeta=planeta, budynek__lata__gt=0, budynek__in=id_statkow).order_by("budynek")
    id_statkow = []
    for tmp in statki_do_wyslania:
        i = GraObject.cache_obj.get_flota_p(planeta.pk, tmp)
        try:ilosc = int(ship_request[i.budynek_id])
        except:ilosc = 0
        if ilosc <= i.ilosc and ilosc > 0:
            id_statkow.append(i.budynek_id)
            true_speed = int(get_speed(GraObject, i.budynek, user))
