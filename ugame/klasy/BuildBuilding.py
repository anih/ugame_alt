...
class BuildBuilding():
        ...
        if planeta:
            ...
            if build.mozna:
                ...
                if build.mozna:
                    ...
                    kolejka = Budynki_f.objects.create(budynek=budynek, planeta=planeta, level=build.level, time=czas + build.c_czas, points=build.points, czas_start=czas)
    ...
    def anuluj_budynek(self, id_budynek):
        ...
        try:
            ...
            bud_kol = Budynki_f.objects.select_for_update().filter(planeta=planeta, budynek=id_budynek).order_by("-level")[0]
        ...
        if max_level:
            ...
            kolejka = Budynki_f.objects.values_list('id', flat=True).select_for_update().filter(planeta=planeta, time__gte=c_czas).order_by("time")
            print kolejka
            if len(kolejka) > 0:
                try:
                    print "try"
                    poprzednie = Budynki_f.objects.select_for_update().filter(planeta=planeta, time__lt=c_czas).order_by("-time")[0]
                    print "poprzednie", poprzednie.time
                    c_czas = c_czas - poprzednie.time
                except:
                    c_czas = c_czas - time()
                print "new c_czas:", c_czas
                for pk in kolejka:
