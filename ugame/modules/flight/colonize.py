...
from ugame.funkcje import check_free_planet, get_speed, check_mamy_kolonizator, get_ship_request, check_ship_request, \
    getCzasLotu, odleglosc, get_zuzycie_deuteru, check_surowce
from ugame.generic.cms_metaclass import CmsMetaclass
from ugame.klasy.BaseGame import Output
from ugame.models import Flota_p, Galaxy, Fleets, send_error_message
...
class CMS(object):
    ...
    def site_choose(self):
        ...
        if not check_free_planet(self.game.user, dane.planeta_id, dane.system_id, dane.galaktyka_id):
            return HttpResponseRedirect("/game/galaxy/")
        if 'ship_request' in self.request.session:
    ...
    def site_accept(self):
        ...
        if not check_free_planet(self.game.user, planeta_dane.planeta_id, planeta_dane.system_id,
                                 planeta_dane.galaktyka_id):
            return HttpResponseRedirect("/game/galaxy/")
        if not check_mamy_kolonizator(self.game.user, current_planet):
            return HttpResponseRedirect("/game/overview/")
        ...
        if not id_statkow or not ship_request:
            tmp = get_ship_request(self.game, current_planet, self.request, self.game.user, planeta_dane, True)
        else:
            tmp = get_ship_request(self.game, current_planet, self.request, self.game.user, planeta_dane, True, False,
                                   False)
    ...
    def site_send(self):
        ...
        tmp = check_surowce(self.request, current_planet, dane_floty)
        ...
        flota_row = Fleets.objects.create(fleet_owner=self.game.user, fleet_mission=1, fleet_amount=fleet_amount,
                                          ...
                                          fleet_resource_metal=zab_met, fleet_resource_crystal=zab_cry,
                                          fleet_resource_deuterium=zab_deu, bak=dane_floty.zuzycie_deuter)
