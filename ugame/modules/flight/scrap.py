...
from ugame.funkcje import check_mamy_recycler, check_ship_request, getCzasLotu, get_ship_request, get_speed, \
...
class CMS(object):
    ...
    def site_choose(self):
        ...
        dostepne_statki_tmp = Flota_p.objects.values_list('budynek_id', flat=True).filter(planeta=current_planet,
                                                                                          budynek__lata__gt=0,
                                                                                          ilosc__gt=0,
                                                                                          budynek__recycler__isnull=False).order_by(
            "budynek")
        dostepne_statki = []
    ...
    def site_accept(self):
        ...
        if not check_mamy_recycler(self.game.user, current_planet):
            return HttpResponseRedirect("/game/overview/")
        ...
        if not id_statkow or not ship_request:
            tmp = get_ship_request(self.game, current_planet, self.request, self.game.user, planeta_dane, False,
                                   True)
        else:
            tmp = get_ship_request(self.game, current_planet, self.request, self.game.user, planeta_dane, False,
                                   True, False)
    ...
    def site_send(self):
        ...
        flota_row = Fleets.objects.create(fleet_owner=self.game.user, fleet_mission=5, fleet_amount=fleet_amount,
